...
class MessageCreate extends index_1.Event {
    ...
    async run(message) {
        ...
        if (mention.test(message.content)) {
            await message.reply({
                content: (0, I18n_1.T)(locale, "event.message.prefix_mention", {
                    prefix: guild?.prefix,
                }),
            });
            return;
        ...
        if (!command)
            return;
        const ctx = new index_1.Context(message, args);
        ctx.setArgs(args);
        ...
        if (command.permissions) {
            ...
            if (command.permissions?.user) {
                const userRequiredPermissions = Array.isArray(command.permissions.user)
                    ...
                    : [command.permissions.user];
                if (!(isDev ||
                    message.member.permissions.has(userRequiredPermissions))) {
                    return await message.reply({
        ...
        if (command.player) {
            if (command.player.voice) {
                if (!message.member.voice.channel) {
                    return await message.reply({
                        content: (0, I18n_1.T)(locale, "event.message.no_voice_channel", {
                ...
                if (!clientMember.permissions.has(discord_js_1.PermissionFlagsBits.Connect)) {
                    return await message.reply({
                        content: (0, I18n_1.T)(locale, "event.message.no_connect_permission", {
                            command: command.name,
                        }),
                    });
                }
                if (!clientMember.permissions.has(discord_js_1.PermissionFlagsBits.Speak)) {
                    return await message.reply({
                        content: (0, I18n_1.T)(locale, "event.message.no_speak_permission", {
                            command: command.name,
                        }),
                    });
                }
                if (message.member.voice.channel?.type ===
                    discord_js_1.ChannelType.GuildStageVoice &&
                    !clientMember.permissions.has(discord_js_1.PermissionFlagsBits.RequestToSpeak)) {
                    return await message.reply({
                        content: (0, I18n_1.T)(locale, "event.message.no_request_to_speak", {
                            command: command.name,
                        }),
                    });
                }
            ...
            if (command.player.dj) {
                ...
                if (dj?.mode) {
                    ...
                    if (!(isDev ||
                        (hasDJRole &&
                            !message.member.permissions.has(discord_js_1.PermissionFlagsBits.ManageGuild)))) {
                        return await message.reply({
        ...
        if (timestamps.has(message.author.id)) {
            ...
            if (now < expirationTime && timeLeft > 0.9) {
                return await message.reply({
                    content: (0, I18n_1.T)(locale, "event.message.cooldown", {
        ...
        try {
            ...
            await message.reply({
                content: (0, I18n_1.T)(locale, "event.message.error", {
            ...
            if (logs) {
                const embed = new discord_js_1.EmbedBuilder()
                    ...
                    .addFields({ name: "Command", value: `\`${command.name}\``, inline: true }, {
                    name: "User",
