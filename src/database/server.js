...
export default class ServerData {
  ...
  async get_247(guildId) {
    if (guildId) {
      const stay = await ServerData.prisma.stay.findUnique({
        where: { guildId },
      });
      if (stay) return stay;
      return null;
  ...
  // No-prefix user management methods

  /**
   * @param {string} userId
   * @param {string} addedBy
   */
  async addNoPrefixUser(userId, addedBy) {
    await ServerData.prisma.noPrefixUser.create({
      data: {
        userId,
        addedBy,
        addedAt: new Date(),
  ...
  async removeNoPrefixUser(userId) {
    await ServerData.prisma.noPrefixUser.delete({
      where: { userId },
    });
  }
  ...
  async isNoPrefixUser(userId) {
    const user = await ServerData.prisma.noPrefixUser.findUnique({
      where: { userId },
    });
    return !!user;
  }
  ...
  async getNoPrefixUsers() {
    return await ServerData.prisma.noPrefixUser.findMany({
